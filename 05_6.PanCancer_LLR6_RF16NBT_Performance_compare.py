...
#Run command, e.g.: python 05_6.PanCancer_LLR6_RF16NBT_Performance_compare.py 1
###############################################################################################
...
if __name__ == "__main__":
    plot_type_list = ['train', 'test', 'delta'] # train CV delta
    plot_type_ind = int(sys.argv[1]) # 1: plot performance on CV, 2: plot delta performance between train and cv
    plot_train_test  = plot_type_list[plot_type_ind]  # train CV delta
    ...
    fig1.subplots_adjust(left=0.08, bottom=0.3, right=0.97, top=0.96, wspace=0.3, hspace=0.55)
    barWidth = 0.3
    ...
    print('%s x1, x2, pval: %.3f, %.3f, %g'%(x_str, np.mean(y1), np.mean(y2), w_pval))
    if sig_str:
    ...
    ax.set_xticklabels(['AUC','PRAUC','Accuracy','F1','MCC','BA','Performance'], rotation=45)

    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)

    fnOut = '../../03.Results/PanCancer_LLR6_RF16NBT_Performance_compare_'+plot_train_test+'.pdf'
    plt.savefig(fnOut)
